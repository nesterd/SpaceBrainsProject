...
class Pages(Resource):
    ...
    def get(self, id=None, name=None):
        ...
        if stat:
            return stat.json(current_identity.admin)
...
class StatList(Resource):
    ...
    def get(self):
        ...
        if result:
            return {'base statistic': [PageModel.find_by_id(el.id).json(current_identity.admin) for el in result if PageModel.find_by_id(el.id)]}
...
class RankList(Resource):
    ...
    def get(self):
        result = SiteModel.query.all()
        if result:
            return {'base_rank statistic': [RankModel.find_by_id(el.id).json() for el in result if RankModel.find_by_id(el.id)]}
...
class RankDayList(Resource):
    ...
    def get(self, date=None):
        ...
        if result:
            return {'base_rank_day statistic - ' + date: [RankModel.find_by_id_day(el.id, date).json_day(date) for el in SiteModel.query.all() if RankModel.find_by_id_day(el.id, date)]}  
...
class RankTimeList(Resource):
    ...
    def get(self, date1=None, date2=None):
        result = PageModel.query.filter(PageModel.scan >= date1, PageModel.scan <= date2).first()
        if result:
            return {'base_rank_time statistic - ' + date1 + '/' + date2: [RankModel.find_by_id_time(el.id, date1, date2).json_time(date1, date2) for el in SiteModel.query.all() if RankModel.find_by_id_time(el.id, date1, date2)]}  
