...
class SiteModel_for_json(SiteModel):
    def _query(self, id, site_id, date, date1, date2):
        ...
        if date1 and date2:
            return query.filter(RankModel.person == id, SiteModel.id == site_id, func.DATE(PageModel.scan) >= date1, func.DATE(PageModel.scan) <= date2)
        if date:
            return query.filter(RankModel.person == id, SiteModel.id == site_id, func.DATE(PageModel.scan) == date)
        return query.filter(RankModel.person == id, SiteModel.id == site_id)
    
    def _get_rank_for_person(self, id, site_id, date=None, date1=None, date2=None):
    ...
    def json_time(self, date1, date2):
        if PageModel.query.filter(PageModel.scan >= date1, PageModel.scan <= date2).first():
            return {'id': self.id,
                    'name': self.name,
                    'persons': [{'id': el.id,
                                 'name': el.name,
...
class RankModel(db.Model):
    ...
    def find_by_id_time(cls, id, date1, date2):
        site = SiteModel_for_json.query.filter_by(id=id).first()
        date = PageModel.query.filter(PageModel.scan >= date1, PageModel.scan <= date2).first()
        if site and date:
            return site
